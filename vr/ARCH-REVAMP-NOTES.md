...
- gfx906 DP4A roofline (MI50-0, 225W, ~1.4-1.5 GHz sustained):
  3840 lanes x 4 INT8 MAC x ~1.45e9 ~ 22 TMAC/s ~ **~44 INT8 TOP/s**.
...
- OPEN ITEM: confirm dense vs MoE. `strings | grep exps` timed out on the ~big
  ...
  `MUL_MAT_ID`-dominated and the repack in section 5.1 must cover the expert path too;
...
3. **For the ne11<=256 range that stays on MMQ:** double-buffered LDS staging
   (section 5.2) and heavier register-blocking of the fat int8 weight tile (more DP4A
...
Q8_0 TG is hard bandwidth bound: ~27B x 1.06 B = ~28.7 GB/token-pass; at ~700
