...
print_optimal_state() {
    cat <<EOF
Current review candidate includes:
  - Q8_0 selective rocBLAS dispatch for wide dense prefill.
  - Q8_0 MMVQ wide-VDR code retained but disabled after a neutral result.
  - Q4_K MMQ metadata precompute plus stride-9 LDS layout in a regressing combined PP set.
  - Q6_K MMQ min_blocks=1 in the same combined set; current-head effect unresolved.
  - Q4_K MMVQ branch-free scale decoding plus Q8_1 sum reuse, retained with a result caveat.
...
verify_compile_gate() {
    ...
    for define in "${OPTIMAL_DEFINES[@]}"; do
        if ! grep -q "$define" "$compile_commands"; then
            ...
            echo "The build would not include all current gfx906 review specializations." >&2
            exit 2
        fi
    done
}
